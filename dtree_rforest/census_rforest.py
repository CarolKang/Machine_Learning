...
"""
    Census using random forest.
    @author: Wenjing Kang

"""

data = sio.loadmat('hw5_data/census_data/census_data.mat')
...
class BinaryDecisionTree:
	def __init__(self, leaf_entropy, max_depth = None, min_samples = 2, sampling_method = range):
		...
		# self.leaf_entropy_threshold = params.get('leaf_entropy_threshold', a)
...
# Random Forest
classifier = RandomForest(100, 0.01, max_depth= 60, \
...
with open('wenjing_cencus_forest.csv', 'w') as csvfile:
      writer = csv.writer(csvfile)
      writer.writerow(['Id','Category'])
      for i in range(len(kaggle_predictions)):
          writer.writerow([i+1, int(kaggle_predictions[i])])
